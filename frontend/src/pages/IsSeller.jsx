...
const IsSeller = () => {
  const { addProduct, loading } = ProductData();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    // Combine checked and other categories
    ...
    if (!form.name || !form.newprice || !form.details || categories.length === 0 || !form.quantity || !form.image || !form.timeToDeliver) {
      toast.error('Please fill all required fields, select/add at least one category, enter delivery time in days, and upload an image.');
      return;
    }
    ...
    const result = await addProduct({ ...form, category: categories, timeToDeliver: `${form.timeToDeliver} days` });
    if (result.success) {
  ...
  return (
    <div className="amazon-add-product-container">
      <h2 className="amazon-add-product-title">Add New Product</h2>
      <form className="amazon-add-product-form" onSubmit={handleSubmit}>
        ...
        <div className="amazon-add-product-field">
          ...
          <input id="image-input" type="file" name="image" accept="image/*" onChange={handleChange} className="amazon-add-product-input" required />
          {imagePreview && (
            <img
              src={imagePreview}
              alt="Preview"
        ...
        <button type="submit" className="amazon-add-product-btn" disabled={loading}>{loading ? 'Adding...' : 'Add Product'}</button>
      </form>
    </div>
  );
};
